...
<a href = "accueil.html"> <p align="center"><img src ="logo1.png"> </p></a>
 ...
 <body>
  ...
  <!-- CORPS DE LA PAGE  -->
  <h2>Bienvenue chez La Rivière</h2></br>
  <form method = "post" action = "la_riviere.php"><br>
  <input type = "submit" name= "new_commande" value="Commander" class ="button"/><br>

  </form>
  <?php
  if (isset($_POST['new_commande'])){
    $id_rest = 6;
    ...
    $req_matricule_sql = " SELECT matricule FROM livreur,client WHERE livreur.ville = client.ville AND id_cli = :id_client AND etat = 'En Ligne'";
    $req_matricule = $cnx->prepare($req_matricule_sql);
    $req_matricule->execute(array(':id_client' =>$id_cli));
    ...
    foreach($array_plats as $plat){
      echo "<div class=\"box2\"><p>Nom :".$plat['nom_plat']." || Prix :".$plat['prix']." </br>
           Description :".$plat['description_plat']." </br></p></div>";
      echo "<form method = 'post' action = 'la_riviere.php'>
            <input type = 'number' name ='quantite' min ='1' max = '10' placeholder = 'Quantité' class ='input_quantite'/></br>
            <input type = 'submit' name ='ajouter_au_panier' value = 'Ajouter ce plat ?' class = 'button'/></br>
            </form></br>";
  ...
  <?php
       $req_info_restos = $cnx->query("SELECT adresse,ville,horaire_ouverture,horaire_fermeture,prix_livraison  FROM restaurant WHERE nom_rest = 'La riviere'");
       ...
       foreach($infos_array as $info_resto){
        echo "<div class=\"box2\"><p> Adresse :".$info_resto['adresse']." </br>
         ...
         Prix de la livraison : ".$info_resto['prix_livraison']."</p></br></a></div>";
