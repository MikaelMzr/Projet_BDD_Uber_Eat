 ...
 <body>
  ...
  if(isset($_POST['envoi_recherche'])){
       ...
       if ($_POST['type_recherche'] == "note"){
          $req_note = $cnx->query('SELECT page_web,nom_rest FROM commenter NATURAL JOIN restaurant ORDER BY note');
       ...
       if ($_POST['type_recherche'] == "nb_avis"){
          ...
          $req_nb_avis = $cnx->query("SELECT page_web ,nom_rest,count(note) AS nb_note FROM restaurant NATURAL JOIN commenter GROUP BY page_web,nom_rest,note ORDER BY note;");
